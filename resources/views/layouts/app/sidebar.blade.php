...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:sidebar sticky collapsible="mobile" class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:sidebar.nav>
                <flux:sidebar.group :heading="__('Traffic System')" class="grid">
                    @if (auth()->user()->isAdmin())
                        <flux:sidebar.item icon="home" :href="route('admin.dashboard')" :current="request()->routeIs('admin.dashboard')" wire:navigate>
                            {{ __('Admin Dashboard') }}
                        </flux:sidebar.item>

                        <flux:sidebar.item icon="clipboard-document-check" :href="route('admin.reports.index')" :current="request()->routeIs('admin.reports.*')" wire:navigate>
                            {{ __('Review Reports') }}
                        </flux:sidebar.item>
                    @else
                        ...
                        <flux:sidebar.item icon="camera" :href="route('reports.create')" :current="request()->routeIs('reports.create')" wire:navigate>
                            {{ __('Report Offence') }}
                        </flux:sidebar.item>
            ...
            <x-desktop-user-menu class="hidden lg:block" :show-team="false" :name="auth()->user()->name" />
        </flux:sidebar>
                ...
                <flux:menu>
                    ...
                    <flux:menu.radio.group>
                        @if (auth()->user()->isAdmin())
                            ...
                            <flux:menu.item :href="route('admin.reports.index')" icon="clipboard-document-check" wire:navigate>
                                {{ __('Review Reports') }}
                            </flux:menu.item>
                        @else
                            ...
                            <flux:menu.item :href="route('reports.create')" icon="camera" wire:navigate>
                                {{ __('Report Offence') }}
                            </flux:menu.item>
